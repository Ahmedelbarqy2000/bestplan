...
const App = () => {
  ...
  const handleTimeWarp = () => {
    const confirm = window.confirm("هل تريد ترحيل الخطة؟ سيتم إزاحة التواريخ يومين للأمام (محاكاة).");
  ...
  const Dashboard = () => {
    if (!planData || planData.length === 0) return <div className="p-10 text-center text-gray-500">Loading Plan Data... check data.js</div>;
